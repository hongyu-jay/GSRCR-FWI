function [x,out]=lbfgs(CostFunction,x0,opts,v_true,w,dom,m0)
tic;
...
J=zeros(Niter,1);% stores the values of the costng=zeros(Niter,1);% stores the values of the norm of the gradient 
Re=zeros(Niter,1);% rmse
...
for i=1:Niter
    ...
    [t,xp,Jxp,gFxp,nInnerCall,JRt]=LineSearchVincent(x,p,Jx,gFx,CostFunction,optsLineSearch);
    step(i)=t;
    if nInnerCall>7
...
%    save('profile','x')
    ...
    hist(:,i)=x;
    Rm=RMS( dom.vec2mat(sqrt(1./abs(m0+w*x))),v_true);
    Re(i)=Rm;
    J(i)=Jxp;
%     save('misfit','J')
    ...
    t2=toc;
    time(i)=t2;
    fprintf('Iteration %d, Jk=%.3d time %.2fs ',i,J(i),t2-t1);
    fprintf('|gk|/|g0| = %.2d\n',ng(i)/ng0);
...
function [test,xt,qt,gt,nInnerCall,JRt]=LineSearchVincent(x,d,q0,g0,CostFunction,opts)    
...
while go
    test=t;
    xt=x+t*d;
    [qt,gt,JRt]=CostFunction(xt);% computes cost and gradient at x+t*d
